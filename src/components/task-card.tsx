...
export default function TaskCard({ task, onClick, isKanban = false, isDraggable = false }: TaskCardProps) {
  ...
  const prChecklistCompleted = task.prChecklist.filter(item => item.checked).length;
  ...
  const hasPrTemplateData = prTemplateData && 
    (prTemplateData.description || prTemplateData.jiraTicket || prTemplateData.testingPlan);
    ...
    >
      <CardHeader className={`${isKanban ? 'p-3 pb-1' : 'pb-2'}`}>
        <div className="flex justify-between items-start">
          <CardTitle className={`${isKanban ? 'text-base leading-tight' : 'text-xl'} group-hover:text-primary transition-colors`}>
            {task.title}
          </CardTitle>
          {!isKanban && (
            <Badge variant={getStatusVariant(task.status) as "investigation" | "planning" | "in-progress" | "in-testing" | "in-review" | "done" | "default"}>
      ...
      <CardContent className={isKanban ? 'p-3 pt-0' : ''}>
        <p className={`text-sm text-muted-foreground ${isKanban ? 'line-clamp-2 mb-2' : 'line-clamp-3 mb-4'}`}>
          {task.description}
        </p>        {/* PR Checklist Progress (if items exist) */}
        {task.prChecklist.length > 0 && (
          <div className="mt-2 mb-2">
            ...
            <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
              <div 
                className={`h-full rounded-full ${prChecklistPercentage === 100 ? 'bg-green-500' : 'bg-primary'}`}
              ...
              <div className="flex justify-between items-center mt-1.5 text-xs">
                ...
                <span 
                  className={`px-1.5 py-0.5 rounded ${
                    hasPrTemplateData ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'
                  }`}
                >
            ...
            {task.status === TaskStatus.IN_REVIEW && prChecklistPercentage < 100 && (
